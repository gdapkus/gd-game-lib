...
- **BGG user registry**: `public/bggUsers.json` lists known users (`name`, `username`, `userid`, `altname`, `color`, `avatarUrl`). Used by server routes to drive collection fetches, and served directly as a static asset for read-only deployments.
