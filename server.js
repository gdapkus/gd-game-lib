...
async function loadCollection(userName) {
  ...
  const retryDelays = [10000, 30000]; // Retry delays: 10s then 30s
  let retries = 0;
  ...
  while (retries <= maxRetries) {
    try {
      ...
      console.log(`BGG response for ${userName} (attempt ${attempt}): status ${response.status}`);
      if (typeof response.data === 'string') {
        const preview = response.data.slice(0, 200).replace(/\s+/g, ' ').trim();
        console.log(`BGG response snippet: ${preview}${response.data.length > 200 ? '…' : ''}`);
      } else {
        console.log(`BGG response keys: ${Object.keys(response.data)}`);
      }
      lastResponseData = response.data;

      const isQueued = response.status === 202 || (typeof response.data === 'string' && response.data.includes('Your request for this collection has been accepted'));
      if (isQueued) {
        retries += 1;
        if (retries > maxRetries) {
          throw new Error(`Collection still queued after ${retries} attempts.`);
        }
        const delay = retryDelays[Math.min(retries - 1, retryDelays.length - 1)];
        console.warn(`BGG collection request queued for ${userName}; requested file delayed, waiting ${delay / 1000}s before retry ${retries}`);
        await wait(delay);
        continue;
      }

      const result = await xml2js.parseStringPromise(response.data); // Parse XML response
      const items = result.items && result.items.item ? result.items.item : null;
      if (!Array.isArray(items) || items.length === 0) {
        retries += 1;
        if (retries > maxRetries) {
          throw new Error(`Collection response missing items after ${retries} attempts.`);
        }
        const delay = retryDelays[Math.min(retries - 1, retryDelays.length - 1)];
        const queueMessage = Array.isArray(result.message) ? result.message.join(' ').trim() : 'Collection not ready yet';
        console.warn(`BGG collection response for ${userName} missing items (${queueMessage}); requested file delayed, waiting ${delay / 1000}s before retry ${retries}`);
        await wait(delay);
        continue;
      }
      console.log(`Received ${items.length} games in collection for ${userName}`);
            ...
            for (const game of items) {
                const existingGame = existingGamesMap.get(game.$.collid);
                const status = (Array.isArray(game.status) && game.status[0] && game.status[0].$) || {};
                const nameValue = readNodeText(game.name, existingGame?.name || 'Unknown title');
                const imageValue = readNodeText(game.image, existingGame?.image || FALLBACK_IMAGE_URL);
                const thumbnailValue = readNodeText(game.thumbnail, existingGame?.thumbnail || FALLBACK_IMAGE_URL);
                const numPlaysValue = readNodeText(game.numplays, existingGame?.numplays || '0');

                if (!Array.isArray(game.image) || !game.image.length || !Array.isArray(game.thumbnail) || !game.thumbnail.length) {
                    console.warn(`Missing artwork for ${nameValue} (${game.$.objectid}); using fallback image.`);
                }
	...
	console.log(`Cache modified: ${cacheModified}`);
      ...
      if (error && typeof error.message === 'string' && error.message.includes("Cannot read properties of undefined (reading '0')")) {
        if (lastResponseData) {
          try {
            const rawResponseDir = path.join(cacheDir, 'rawResponses');
            fs.mkdirSync(rawResponseDir, { recursive: true });
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const rawFilePath = path.join(rawResponseDir, `collection_${sanitizedUserName}_${timestamp}.xml`);
            const serializedData = typeof lastResponseData === 'string'
              ? lastResponseData
              : JSON.stringify(lastResponseData, null, 2);
            fs.writeFileSync(rawFilePath, serializedData, 'utf8');
            console.error(`Saved raw BGG response to ${rawFilePath}`);
          } catch (fileError) {
            console.error(`Failed to save raw BGG response: ${fileError.message}`);
          }
        }
        throw error;
      }
      ...
      const delay = retryDelays[Math.min(retries - 1, retryDelays.length - 1)];
      console.warn(`Waiting ${delay / 1000}s before retry ${retries} for ${userName}`);
      await wait(delay);
    }
  }
}
...
function readNodeText(node, fallback = null) {
