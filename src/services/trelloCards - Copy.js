...
export async function getTrelloLists(trelloToken) {
    try {
        const response = await axios.get(`https://api.trello.com/1/boards/${process.env.VITE_TRELLO_BOARD_ID}/lists`, {
            params: {
                key: process.env.VITE_TRELLO_KEY,
                token: trelloToken
            }
        });
...
export async function createTrelloCard(listId, gameId, trelloToken) {
...
//module.exports = { createTrelloCard, getTrelloLists };